...
/*   Updated: 2025/03/13 00:35:31 by f4t4lity         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int	i = 0, int	primeira_palavra = 1;
	...
	while (str[i] != '\0')
	{
		if (isalnum(str[i])) {
			if (primeira_palavra)
			{
				str[i] = toupper(str[i]), primeira_palavra = 0;
			
			} else {
				str[i] = tolower(str[iI]);
			} else {
				primeira_palavra = 1;
				}
				i++;
